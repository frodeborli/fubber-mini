...
echo "Testing Session write/delete failure handling...\n";
...
echo "  ✓ set() throws when the cache backend refuses the write\n";
...
assert_eq(null, $session->get('user_id'), 'A refused write must not be readable afterwards');
assert_eq([], $session->all(), 'all() must not expose data that was never stored');
assert_false($session->has('user_id'), 'has() must not report a refused write');
assert_eq(0, count($session), 'count() must not include a refused write');
...
echo "  ✓ remove()/clear()/regenerate() throw on a refused write\n";

// A refused write left nothing behind
assert_eq([], $cache->store, 'Nothing should have been stored by a refused write');

// Once the backend accepts writes again, save() persists the pending data
$cache->writable = true;
$ok = new Session($cache);
$ok->set('user_id', 123);
assert_eq(123, $ok->get('user_id'), 'Accepted write should be readable');
assert_true($cache->has('session.' . $ok->getId()), 'Accepted write should reach the cache');
echo "  ✓ Writes succeed normally once the backend accepts them\n";
...
assert_eq(123, $ok->get('user_id'), 'A refused write must leave the last persisted value visible');
assert_eq(['user_id' => 123], $ok->all(), 'all() must reflect the store, not the refused write');
$cache->writable = true;
echo "  ✓ A refused write falls back to the last persisted state\n";
...
// 1. destroy() on a session that was never written to cache
$never = new Session(new StubCache());
assert_true($never->destroy(), 'destroy() on a never-persisted session must succeed');
echo "  ✓ destroy() on a never-persisted session does not throw\n";

// 2. Double logout / session evicted by the store between requests
$twice = new Session($cache);
$twice->set('user_id', 7);
$key = 'session.' . $twice->getId();
assert_true($cache->has($key), 'Session should be in cache before destroy()');
assert_true($twice->destroy(), 'First destroy() should succeed');
assert_false($cache->has($key), 'destroy() should remove the session data');
assert_true($twice->destroy(), 'Second destroy() (double logout) must not throw');
echo "  ✓ destroy() is idempotent (double logout / evicted session)\n";

// 3. regenerate(deleteOldSession: true) when the old session was never stored
$regen = new Session($cache);
$oldId = $regen->getId();
assert_false($cache->has('session.' . $oldId), 'Old session was never persisted');
assert_true($regen->regenerate(deleteOldSession: true), 'regenerate() must not throw on an unpersisted old session');
assert_true($regen->getId() !== $oldId, 'regenerate() should produce a new ID');
echo "  ✓ regenerate(deleteOldSession: true) tolerates an unpersisted old session\n";
...
assert_true($hostile->has($liveKey), 'Session should be stored before the refused delete');
...
assert_true($hostile->has($liveKey), 'The refused delete left the session readable — that is the failure');
echo "  ✓ destroy() throws when a refused delete leaves session data readable\n";
...
echo "  ✓ regenerate(deleteOldSession: true) throws when the old session survives\n";
...
$containerCache = new StubCache();
\mini\Mini::$mini->set(CacheInterface::class, $containerCache);

$default = new Session();
$default->set('from_container', 'yes');
$defaultKey = 'session.' . $default->getId();
assert_true($containerCache->has($defaultKey), 'new Session() must resolve CacheInterface from the container');
assert_eq('yes', $default->get('from_container'), 'Container-backed session should read back its own write');
assert_eq(['from_container' => 'yes'], $default->all(), 'all() should expose container-backed data');
assert_true($default->destroy(), 'Container-backed session should destroy cleanly');
assert_false($containerCache->has($defaultKey), 'destroy() should remove the container-backed session');
echo "  ✓ new Session() resolves its cache lazily from the container\n";

echo "\nAll Session write/delete failure tests passed!\n";
